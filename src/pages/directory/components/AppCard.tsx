...
export default function AppCard({ app, viewMode, viewDetailsText = 'View Details' }: AppCardProps) {
    ...
    return (
      <div className="flex items-center gap-1">
        {[...Array(fullStars)].map((_, i) => (
          <i key={`full-${i}`} className="ri-star-fill text-yellow-400 text-sm" />
        ))}
        {hasHalfStar && <i className="ri-star-half-fill text-yellow-400 text-sm" />}
        {[...Array(emptyStars)].map((_, i) => (
          <i key={`empty-${i}`} className="ri-star-line text-gray-300 text-sm" />
        ))}
  ...
  if (viewMode === 'list') {
    return (
      <Link to={`/products/${app.slug}`} className="block bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-4 md:p-6 border border-gray-100 hover:border-gray-200">
        <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-semibold font-poppins w-fit ${getPricingColor(pricing)}`}>
                {pricing}
              </span>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:gap-2">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                ...
                <span className="w-full sm:w-auto inline-block text-center bg-[#1F2853] text-white px-4 py-2 rounded-lg text-sm font-medium font-poppins">
                  {viewDetailsText}
                </span>
              </div>
            </div>
  ...
  return (
    <Link to={`/products/${app.slug}`} className="block bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-4 md:p-6 border border-gray-100 hover:border-gray-200 group">
      {app.featured && (
        <div className="mb-4">
      ...
      <span className="block w-full text-center bg-[#1F2853] text-white py-3 rounded-lg font-medium group-hover:bg-[#2a3a6b] transition-colors whitespace-nowrap font-poppins">
        {viewDetailsText}
      </span>
    </Link>
  );
