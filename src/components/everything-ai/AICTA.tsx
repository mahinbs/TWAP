...
const AICTA = () => {
    ...
    const titlePart1 = c?.title_part1 ?? 'Unlock the';
    const titlePart2 = c?.title_part2 ?? 'Next Dimension';
    const boldPhrase = c?.bold_phrase ?? 'Create it.';
    const description = section?.description
        ?? "Don't just adapt to the future. Create it. Join the elite circle of AI-native enterprises today.";
    ...
    const descParts = description.split(boldPhrase);
    const hasBoldSplit = descParts.length > 1;
                    ...
                    <div className="relative z-10 max-w-4xl mx-auto"
                        ...
                        <p className="text-gray-500 text-xl md:text-2xl mb-12 max-w-2xl mx-auto font-light leading-relaxed">
                            {hasBoldSplit ? (
                                <>
                                    {descParts[0]}
                                    <span className="text-brand-dark font-medium">{boldPhrase}</span>
                                    {descParts.slice(1).join(boldPhrase)}
                                </>
                            ) : description}
                        </p>
                        ...
                        {section?.cta_text && (
                            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
                                <Link
                                    to={section.cta_url ?? '/services'}
                                    className="px-10 py-5 rounded-full bg-brand-orange text-white font-bold hover:bg-brand-dark transition-all flex items-center gap-3 group shadow-xl shadow-brand-orange/20 hover:scale-105"
                                >
                                    {section.cta_text}
